...
def parse_binary_compressed_pc_data(f, dtype, metadata):
    ...
    print("No support for compressed data for now.")
    sys.exit(1)
...
class PointCloud(object):
    ...
    def to_array_xyz(self):
        num_points = len(self.pc_data)
        result = np.zeros((num_points, 3))
        result[:, 0] = self.pc_data['x']
        result[:, 1] = self.pc_data['y']
        result[:, 2] = self.pc_data['z']
        return result
        # return np.array([list(p)[:3] for p in self.pc_data])
